...
configs/data/*.yaml        -- 数据集配置（boolq / mmlu / bbh / sciq / arc / math / gsm8k）
...
BoolQ 使用独立的 yes_no 模板；MMLU/BBH/SCIQ/ARC 共享多选模板（`BBH_TEMPLATES = MMLU_TEMPLATES`）。MATH 和 GSM8K 使用独立的数学模板，要求答案以 `\boxed{}` 格式输出。

## 测试策略

8 个测试文件覆盖所有核心模块（363 个测试全部通过），使用 `unittest.mock` 隔离外部依赖（HuggingFace 数据加载、vLLM 推理、GLM API 调用）。关键测试模式：
...
`detect_model_type()` 函数通过模型名称字符串匹配判断架构类型：`"llama"` -> llama3，`"mistral"` -> mistral，`"gemma"` -> gemma2，`"qwen"` + `"2.5"` -> qwen2.5，`"qwen"` -> qwen3（默认）。Qwen2.5 检测必须在通用 qwen 之前。
